...
void main(){
  float effectivePixelSize = uPixelSize;
  vec2 fragCoord = gl_FragCoord.xy - uResolution * .5;
  float aspectRatio = uResolution.x / uResolution.y;

  vec2 pixelId = floor(fragCoord / effectivePixelSize);
  vec2 pixelUV = fract(fragCoord / effectivePixelSize);

  float cellPixelSize = 8.0 * effectivePixelSize;
  ...
  if (uEnableRipples == 1) {
    ...
    for (int i = 0; i < MAX_CLICKS; ++i){
      ...
      float cp = 8.0 * effectivePixelSize;
      vec2 cuv = (((pos - uResolution * .5 - cp * .5) / (uResolution))) * vec2(aspectRatio, 1.0);
  ...
  float bayer = Bayer8(fragCoord / effectivePixelSize) - 0.5;
  float bw = step(0.5, feed + bayer);

  float h = fract(sin(dot(floor(fragCoord / effectivePixelSize), vec2(127.1, 311.7))) * 43758.5453);
